...
CREATE INDEX IF NOT EXISTS idx_phase_name ON phase(phase_name);
CREATE INDEX IF NOT EXISTS idx_message_phase ON message(phase_id);
...
# Migrations for existing DBs — SQLite does not support IF NOT EXISTS on ALTER TABLE,
# so we attempt each migration and ignore the error if the column already exists.
_MIGRATIONS = [
