"""github_minimax orchestrator: investigate -> implement -> review+PR.

MiniMax-flavored copy of engines/three_step/orchestrator.py. Routes every LLM
call through engines.github_minimax.runtime (OpenAI SDK against MiniMax's
OpenAI-compat endpoint) — there is no claude_runtime here; the MiniMax
runtime is the only backend. The Claude CLI subscription path is NOT
supported by this engine.

Reuses shared modules from github_openhands for source, storage, workspace,
and destination.
"""
...
def _call_agent(prompt: str, *, phase: str, cwd: str, model: str | None = None) -> dict:
    ...
    The github_minimax engine has only one backend: MiniMax via the OpenAI
    SDK. Any model string — claude/sonnet/opus/haiku aliases for backwards
...
def _call_openai_compat(prompt: str, *, phase: str, cwd: str, model: str) -> dict:
    """Call MiniMax via the OpenAI-SDK runtime in this engine.

    The MiniMax-flavored runtime handles alias resolution (m3 -> MiniMax-M3),
    price lookup against adapters/minimax.py MODELS, API-key resolution with
    ~/.zshrc fallback, and sampling params from models/minimax.py CONFIG.
    """
    import adapters
    import models as model_configs
    from engines.github_minimax import runtime
...
def _call_claude(prompt: str, *, phase: str, cwd: str, model: str | None = None) -> dict:
    """This engine has no Claude CLI path — MiniMax is the only backend.

    Retained as a function so the orchestrator compiles, but it must never
    be called. _call_agent() always routes through _call_openai_compat().
    """
    raise RuntimeError(
        "engines.github_minimax has no Claude CLI backend. "
        "Use the MiniMax-flavored _call_openai_compat() path. "
...
def _run_learning_phase(
    *,
    ...
    resp = _call_agent(prompt, phase="extract_lessons", cwd=wt, model="m3")
    resp["_prompt"] = prompt
...
def run_pipeline(
    ...
    *,
    ...
    """Run the github_minimax pipeline: investigate -> implement -> review+PR.

    Returns dict with keys: status, pr_url, spent_usd, run_id, error.

    Args:
        workflow_dir: optional override for the workflow dir (default: workflows/issue-to-pr)
        phases: optional comma-separated phase list (e.g. "triage,verify"). Default: all
            phases from workflow.yaml. Spec'd by the smoke-test in the task brief.
    """
    ...
    branch = f"github_minimax/issue-{issue_number}-{run_start.strftime('%m%d-%H%M')}"
    if not repo_path:
    ...
    try:
        ...
        try:
            ...
            audit_resp = _call_agent(prompt, phase="audit", cwd=wt, model="m3")
